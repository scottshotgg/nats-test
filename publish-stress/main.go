...
func createPublisher(subjects []string, sc stan.Conn, length, freq time.Duration) error {
	var err error
	...
	for _, subject := range subjects {
		...
		err = sc.Publish(subject, newMessageFunc())
		if err != nil {
			return err
		}
...
func startNats(num int, id string, total time.Duration) error {
	...
	idSub, err := sc.Subscribe(id, func(m *stan.Msg) {
		fmt.Printf("%s %d: %s\n", id, num, string(m.Data))
	...
	log.Println("Client " + id + " subscribed to subject " + id + "!")

	// Make the client subscribe to a "mailbox" with the same name as the client ID
	allSub, err := sc.Subscribe("all", func(m *stan.Msg) {
		fmt.Printf("all %d: %s\n", num, string(m.Data))
	}, stan.DurableName("durable"+id))
	if err != nil {
		fmt.Println("sc.Subscribe")
	...
	log.Println("Client " + id + " subscribed to subject all!")
...
func main() {
	...
	err = createPublisher(append(ids, "all"), sc, length, freq)
	if err != nil {
		log.Fatal("err", err)
	}
